...
This document lists and describes all variables included in the **Early Warning System (EWS)** dataset.  
Each feature includes its source, data type, a concise description, and its analytical purpose within the model.
...
- All data merged at the school level using `cdscode` as the unique key.  
- County-level safety and connectedness data were joined by normalized county name.  
- Percentage values are expressed as decimals (e.g., 0.75 = 75%).  
- Missing or suppressed data reflect privacy restrictions or small cohort counts.  
